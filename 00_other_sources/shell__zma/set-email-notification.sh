...
if [ $# -lt 1 ]; then
    echo -e "Usage: $0 repo_dir_name"
    echo -e "Example: $0 test.git"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    echo -e "Hooks in config already set. skip"
else
    echo -e -e "[hooks]\n   mailinglist = \"$receiveremails\"\n   senderemail = \"$senderemail\"" >> config
fi

if [ -e description ]; then
    echo -e "description already exists. skip"
else
    echo -e "$repo" > description
fi

cd hooks
if [ -e post-receive ]; then
    echo -e "post-receive already exists. skip"
else
    ln -s ../../../post-receive ./
fi
