...
echo -e -e "THIS PROGRAM IS OF NO WARRANTY!!! I AM NOT RESPONSIBLE FOR ANY
...
if [ $mail_status != 0 ]
then
	echo -e "I am very sorry... But I require the mailutils package to sendmail"
	echo -e "I suggest using apt or yum to install"
	exit 1
fi

if [ $check_ser_status != 0 ]
then
	echo -e "I am very sorry... But I require the Postfix service"
	echo -e "I suggest using apt or yum to install"
	exit 1
fi
...
# if [ $gui_status == 0 ]
# then
# 	echo -e "WARNING!!! Running on home Computer is highly not recommended due to the increased chance of being mislead as spam"
# 	sleep 2
# 	exit 1
# fi
...
echo -e "Enter message in one second: "
sleep 2
...
if [ $send == "yes" ]
then
	echo -e $body | MAILRC=/dev/null mail -r $from -s "$subject" $receive
	exit 0
else
	echo -e "The body of your message was stored here: $temp_d$random$mail_ext"
	exit 0
fi
