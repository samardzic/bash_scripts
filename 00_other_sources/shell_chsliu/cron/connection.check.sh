...
testhost() {
	...
	# echo -e ping $1, exitcode: $ret
	return $ret
	
	# if [ "$ret" -eq "0" ]; then
		# echo -e $ret=="0", returning 0
		# return 0
	# else
		# echo -e $ret!="0", returning 1
		# return 1
	# fi
}
...
connection_check() {
	...
	if ! testhost $gateway; then
		echo -e gateway disconnected, badconnflag created.
		touch $badconnflag 
	else 
		echo -e gateway connected
		
		if [ -e $badconnflag ] || ! testhost $dns; then 
			echo -e badconnflag found or internet disconnected, reset gateway.
			reset_gateway
			rm $badconnflag
		else 
			...
			if [ ! -e $lastonflag ]; then 
				...
				if [ "$no_reset_hour_start" -lt "$now_hour" ] && [ "$now_hour" -lt "$no_reset_hour_stop" ]; then
					echo -e "$no_reset_hour_start < $now_hour < $no_reset_hour_stop", no reset
				elif [ "$now_min" -eq 0 ] && test `find "$lastonflag" -mmin +$onmin`; then
					echo -e lastonflag is old enough, reset gateway.
					reset_gateway				
				fi
			fi
		fi
