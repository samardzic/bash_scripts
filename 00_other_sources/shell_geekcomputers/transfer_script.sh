...
funct_copy_file()
{
  if [ ! -z $transfer_file ]; then
    ...
    echo -e "Sending $transfer_file to $SERVER in /tmp"
    scp $transfer_file $SERVER:/tmp
  else
    echo -e -e "File does not exist \n"
    echo -e -e "A search for similar files shows :"
    no_ext=`echo -e $script_name | cut -f1 -d.`
    #echo -e $no_ext
