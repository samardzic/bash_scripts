#!/bin/sh
echo -e "show_hme, v1.5. Stacey Marshall, 16 April 1998"
...
#        INSTANCES=1
	;;
*)
	INSTANCES=`echo -e "hmeps/4X" | adb -k /vmunix /dev/mem |
		nawk 'BEGIN { FS = ":"; RS="" }
		{ print \$NF }' | 
		nawk '{for (i = 1; i <= NF; i++) {
		if ($i != 0)
...
get_eri_info() {
  parameters=`echo -e "$1+0x130/29D" | adb -k /vmunix /dev/mem | nawk '/physmem/ { next }
...
# Check that the users id is root 
   ...
   if [ $? -ne 0 ]
        then 
        echo -e "Only the super-user can execute this script " 
        abort 
   fi 
} 
...
echo -e "Detailing Instances: \"${INSTANCES}\""
for instance in $INSTANCES
	do
	$echo -e "\n\nConfiguration of eri card # $instance.\n"
	$echo -e "NOTE Parameters MAY NOT be accurate if the card is not connected to a HUB/SWITCH"
	$echo -e "or if the interface is unused ( IE: Not plumbed in )."
	$echo -e "Try snooping the device to inititalise it."
