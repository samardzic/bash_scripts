...
funct_check_params()				# Start of the procedure
{
  if [ ${NARG} -lt 3 ]; then			# Check there are 3 arguments passed to the script, if not display the help 
    echo -e "Usage: $0 directory *.old_extension *.new_extension"
...
{
  old_ext_cut=`echo -e ${old_ext} | cut -f2 -d .` 	# Cut just the extension passed, lose the .
  new_ext_cut=`echo -e ${new_ext} | cut -f2 -d .`	# Cut just the extension passed, lose the . 
  
  # This will carry out the rename

  for file in $(ls $work_dir/*$old_ext); do mv $file `echo -e $file | sed 's/\(.*\.\)'$old_ext_cut'/\1'$new_ext_cut/` ; done
}
