...
{												# Start of the function
  if [ ${NARG} -ne 1 ]; then			# If the number of arguments is not one, then output a message
    echo -e "$0 : Not enough Parameters passed, you need to supply an ORACLE_SID"
  ...
  [[ ${SID} = "-h" ]] || [[ ${SID} = "--h" ]]; then
    echo -e "Usage: You need to add an ORACLE_SID, e.g $0 BOSSLIVE"
    exit 1									# Quit the program
...
{												# Start of the function
  ...
  echo -e " You have ${numconnections} connections to the database ${SID}"										# Display the amount of connections to the database
}												# End of the function
